from deps.data_access import data_access_get_guild_voice_channel_ids, data_access_get_member, data_access_get_voice_user_list
...
async def send_private_notification_following_user(bot: MyBot, joined_user_id: int, guild_id: int, channel_id: int):
    ...
    for user_id in filtered_followed_user_ids:
        ...
        if member and not member.voice:
            ...
            print_log(f"User {user_id} is currently in a voice channel; skipping notification for {joined_user_id}.")
