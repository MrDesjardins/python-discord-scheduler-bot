...
from deps.models import UserFullMatchInfo, UserMatchInfo, UserQueueForStats, UserWithUserMatchInfo
...
async def download_matches(users: List[UserQueueForStats]) -> List[UserWithUserMatchInfo]:
    """
    Download the matche to a group of user
    """
...
async def download_matches_async(
    last_hour: int, users: List[UserQueueForStats], post_process_callback=None
) -> List[UserWithUserMatchInfo]:
    """
    Run the blocking download_matches in another thread and perform a post-processing task
    in the main thread after completion.
    """

    def blocking_task():
        # Convert the async function into a blocking call
        return asyncio.run(download_matches(users))
    ...
    try:
        ...
        if post_process_callback:
            ...
            loop.call_soon(asyncio.create_task, post_process_callback(last_hour, users, matches))

        return matches
    except Exception as e:
        print_error_log(f"download_matches_async: Error during match download: {e}")
        return []
