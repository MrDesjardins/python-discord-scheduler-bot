...
def build_msg_stats_first_death(
    ...
    msg += (
        f"{columnize('#', 3)}"
        f"{columnize('Name', COL_WIDTH)}"
        f"{columnize('Count Death', COL_WIDTH)}"
        f"{columnize('Count Round', COL_WIDTH)}"
        f"{columnize('Rate Dying 1st', COL_WIDTH)}\n"
    )
    for stat in stats_tuple:
        if rank >= TOP:
            break
...
def stats_ace_count(day: int, last_x_day: date) -> str:
    ...
    msg = build_msg_4_counts("ace, 4k, 3k rounds", f"in the last {day} days", stats)
    return msg
