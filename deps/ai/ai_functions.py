...
class BotAI:
    ...
    async def ask_ai_async(self, question: str, timeout: float = 800.0) -> Union[str, None]:
        """
    ...
    def summarize_full_match(self, match: UserFullMatchStats) -> str:
        ...
        """
        summary = f"""
    Start of info for the match information for match id {match.match_uuid} played on {match.match_timestamp.strftime('%Y-%m-%d %H:%M:%S')} for user_id {match.user_id} who also share this r6_tracker_active_id: {match.r6_tracker_user_uuid}.
