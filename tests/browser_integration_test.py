...
@contextmanager
def browser_context_or_skip(*args):
    """Create a browser context, or skip live browser tests when the local browser is unavailable."""
    try:
        with BrowserContextManager(*args) as context:
            yield context
    except (BrowserStartupException, CircuitBreakerOpenException) as e:
        pytest.skip(f"Live browser integration unavailable: {e}")
...
async def test_matches(mock_error_log):
    ...
    with browser_context_or_skip() as context:
...
async def test_highest_rank_diamond(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("noSleep_rb6") as context:
...
async def test_highest_rank_platinum(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("LebronsCock") as context:
...
async def test_highest_rank_user_not_found(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("DoesNotExist123000Name") as context:
...
async def test_highest_rank_champion(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("Funkyshmug") as context:
...
async def test_highest_rank_emerald(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("Adahdf.") as context:
...
async def test_highest_rank_gold(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("J0hn_Th1cc") as context:
...
async def test_highest_rank_emerald_2(mock_error_log):
    ...
    # Sleep to avoid rate limiting
    await asyncio.sleep(DELAY_SECOND)
    with browser_context_or_skip("Yuuka_Kazami") as context:
