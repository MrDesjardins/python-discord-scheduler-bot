...
def test_multiple_connects_same_day_returns_one_date():
    ...
    connect_at(USER_1, GUILD_A, days_ago(0), hour=18)
    connect_at(USER_1, GUILD_A, days_ago(0), hour=20)
    connect_at(USER_1, GUILD_A, days_ago(0), hour=22)
    result = fetch_distinct_play_dates(USER_1, GUILD_A)
    assert result == [days_ago(0)]
...
def test_disconnect_only_does_not_count_as_play_day():
    ...
    disconnect_at(USER_1, GUILD_A, days_ago(0))
    result = fetch_distinct_play_dates(USER_1, GUILD_A)
    assert result == []
...
def test_guild_isolation_different_guild_not_counted():
    ...
    result = fetch_distinct_play_dates(USER_1, GUILD_A)
    assert result == [days_ago(0)]  # Only today from GUILD_A
...
def test_no_users_active_on_date_returns_empty():
    ...
    result = fetch_user_ids_active_on_date(GUILD_A, days_ago(0))
    assert result == []
...
def test_two_users_active_on_same_date():
    ...
    connect_at(USER_1, GUILD_A, days_ago(0))
    connect_at(USER_2, GUILD_A, days_ago(0))
    result = fetch_user_ids_active_on_date(GUILD_A, days_ago(0))
...
def test_guild_isolation_for_active_users():
    ...
    connect_at(USER_1, GUILD_B, days_ago(0))
    result = fetch_user_ids_active_on_date(GUILD_A, days_ago(0))
    assert result == []
...
def test_multiple_connects_same_day_user_appears_once():
    ...
    connect_at(USER_1, GUILD_A, days_ago(0), hour=18)
    connect_at(USER_1, GUILD_A, days_ago(0), hour=21)
    result = fetch_user_ids_active_on_date(GUILD_A, days_ago(0))
    assert result == [USER_1]
...
@pytest.mark.no_parallel
def test_end_to_end_thirty_day_streak():
    """Insert 30 consecutive days of activity and verify a 30-day streak."""
    for i in range(30):
        connect_at(USER_1, GUILD_A, days_ago(i))
