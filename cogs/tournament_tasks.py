...
from deps.tournament_discord_actions import send_tournament_bracket_to_a_guild, send_tournament_registration_to_a_guild

local_tz = pytz.timezone("America/Los_Angeles")
time_send_daily_brackets = time(hour=9, minute=0, second=0, tzinfo=local_tz)
time_send_daily_registration = time(hour=9, minute=10, second=0, tzinfo=local_tz)
...
class TournamentTasksCog(commands.Cog):
    ...
    async def start_task(self):
        ...
        self.send_daily_tournament_bracket_message.start()  # Start the task when the cog is loaded
        self.send_daily_tournament_registration_message.start()  # Start the task when the cog is loaded

    @tasks.loop(time=time_send_daily_brackets)
    async def send_daily_tournament_bracket_message(self):
        """
        Send only once every day the bracket for each guild who has the bot
        """
        print_log(f"send_daily_tournament_bracket_message: Sending daily bracket {datetime.now()}")
        for guild in self.bot.guilds:
            guild_id = guild.id
            await send_tournament_bracket_to_a_guild(guild_id)

    @send_daily_tournament_bracket_message.before_loop
    async def before_send_daily_question_to_all_guild_task(self):
        """Wait for the Send Daily Message Task to have the bot ready"""
        print_log("TournamentTasksCog>send_daily_tournament_bracket_message: Waiting for bot to be ready...")
        await self.bot.wait_until_ready()

    @tasks.loop(time=time_send_daily_registration)
    ...
    async def cog_unload(self):
        self.send_daily_tournament_bracket_message.cancel()
