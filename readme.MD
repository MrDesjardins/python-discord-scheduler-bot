...
1. Create a new bash file in `~` called `do.sh` and make it executable. The goal is to set the environment variables and run the script.
```sh
cd ~
touch do.sh
chmod +x do.sh
...
./python-discord-scheduler-bot/do.sh
```
